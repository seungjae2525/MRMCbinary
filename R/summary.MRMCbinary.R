...
summary.MRMCbinary <- function(object, digits=max(1L, getOption("digits") - 3L), ...) {
  ...
  if (x$effect == "Both") {
    ...
    temp_CLR_sen_p <- format_f(temp_CLR_sen$P.value, numer=NULL, denom=NULL, digits=digits)
    temp_CLR_sen_p <- ifelse(temp_CLR_sen_p == paste0("0.", paste0(rep("0", digits), collapse = "")),
                             paste0(paste0("< 0.", paste0(rep("0", digits-1), collapse = "")), "1"), temp_CLR_sen_p)
    mat_CLR_sen <- data.frame(format_f(temp_CLR_sen$Estimate, numer=NULL, denom=NULL, digits=digits),
                              format_f(temp_CLR_sen$SE, numer=NULL, denom=NULL, digits=digits),
                              paste0("[", format_f(temp_CLR_sen$Lower.ci, numer=NULL, denom=NULL, digits=digits),
                                     ", ", format_f(temp_CLR_sen$Upper.ci, numer=NULL, denom=NULL, digits=digits), "]"),
                              temp_CLR_sen_p)
    ...
    rownames(mat_CLR_sen) <- paste0(c(rep(paste0("Modality", x$reference.Modality), x$n.modality-1),
                                      rep(paste0("Reader", x$reference.Reader), x$n.reader-1)), " vs ", rownames(temp_CLR_sen))
    ...
    temp_test_sen_p <- format_f(temp_test_sen$P.value, numer=NULL, denom=NULL, digits=digits)
    temp_test_sen_p <- ifelse(temp_test_sen_p == paste0("0.", paste0(rep("0", digits), collapse = "")),
                              paste0(paste0("< 0.", paste0(rep("0", digits-1), collapse = "")), "1"), temp_test_sen_p)
    mat_test_sen <- data.frame(format_f(temp_test_sen$Statistic, numer=NULL, denom=NULL, digits=digits),
                               temp_test_sen$DF,
                               temp_test_sen_p)
    ...
    if (x$interaction == TRUE) {
      ...
      temp_Q_MN_sen_p <- format_f(temp_Q_MN_sen$P.value, numer=NULL, denom=NULL, digits=digits)
      temp_Q_MN_sen_p <- ifelse(temp_Q_MN_sen_p == paste0("0.", paste0(rep("0", digits), collapse = "")),
                                paste0(paste0("< 0.", paste0(rep("0", digits-1), collapse = "")), "1"), temp_Q_MN_sen_p)
      mat_Q_MN_sen <- data.frame(format_f(temp_Q_MN_sen$Statistic, numer=NULL, denom=NULL, digits=digits),
                                 temp_Q_MN_sen$DF,
                                 temp_Q_MN_sen_p)
    ...
    temp_CLR_spe_p <- format_f(temp_CLR_spe$P.value, numer=NULL, denom=NULL, digits=digits)
    temp_CLR_spe_p <- ifelse(temp_CLR_spe_p == paste0("0.", paste0(rep("0", digits), collapse = "")),
                             paste0(paste0("< 0.", paste0(rep("0", digits-1), collapse = "")), "1"), temp_CLR_spe_p)
    mat_CLR_spe <- data.frame(format_f(temp_CLR_spe$Estimate, numer=NULL, denom=NULL, digits=digits),
                              format_f(temp_CLR_spe$SE, numer=NULL, denom=NULL, digits=digits),
                              paste0("[", format_f(temp_CLR_spe$Lower.ci, numer=NULL, denom=NULL, digits=digits),
                                     ", ", format_f(temp_CLR_spe$Upper.ci, numer=NULL, denom=NULL, digits=digits), "]"),
                              temp_CLR_spe_p)
    ...
    rownames(mat_CLR_spe) <- paste0(c(rep(paste0("Modality", x$reference.Modality), x$n.modality-1),
                                      rep(paste0("Reader", x$reference.Reader), x$n.reader-1)), " vs ", rownames(temp_CLR_spe))
    base::print(mat_CLR_spe)
    cat("\n")

    cat("Test from conditional logistic regression: \n")
    temp_test_spe <- round(rbind(x$CLR_LRT_spe, x$CLR_Score_spe, x$CLR_Wald_spe), digits=digits)
    temp_test_spe_p <- format_f(temp_test_spe$P.value, numer=NULL, denom=NULL, digits=digits)
    temp_test_spe_p <- ifelse(temp_test_spe_p == paste0("0.", paste0(rep("0", digits), collapse = "")),
                              paste0(paste0("< 0.", paste0(rep("0", digits-1), collapse = "")), "1"), temp_test_spe_p)
    mat_test_spe <- data.frame(format_f(temp_test_spe$Statistic, numer=NULL, denom=NULL, digits=digits),
                               temp_test_spe$DF,
                               temp_test_spe_p)
    ...
    if (x$interaction == TRUE) {
      ...
      temp_Q_MN_spe_p <- format_f(temp_Q_MN_spe$P.value, numer=NULL, denom=NULL, digits=digits)
      temp_Q_MN_spe_p <- ifelse(temp_Q_MN_spe_p == paste0("0.", paste0(rep("0", digits), collapse = "")),
                                paste0(paste0("< 0.", paste0(rep("0", digits-1), collapse = "")), "1"), temp_Q_MN_spe_p)
      mat_Q_MN_spe <- data.frame(format_f(temp_Q_MN_spe$Statistic, numer=NULL, denom=NULL, digits=digits),
                                 temp_Q_MN_spe$DF,
                                 temp_Q_MN_spe_p)
    ...
    colnames(mat_CLR_sen) <- c("Odds ratio", "Standard error", "Confidence interval", "P value")
    if (x$effect == "Modality") {
      rownames(mat_CLR_sen) <- paste0(rep(paste0("Modality", x$reference.Modality), x$n.modality-1), " vs ", rownames(temp_CLR_sen))
    } else {
      rownames(mat_CLR_sen) <- paste0(rep(paste0("Reader", x$reference.Reader), x$n.reader-1), " vs ", rownames(temp_CLR_sen))
    }
    ...
    colnames(mat_CLR_spe) <- c("Odds ratio", "Standard error", "Confidence interval", "P value")
    if (x$effect == "Modality") {
      rownames(mat_CLR_spe) <- paste0(rep(paste0("Modality", x$reference.Modality), x$n.modality-1), " vs ", rownames(temp_CLR_spe))
    } else {
      rownames(mat_CLR_spe) <- paste0(rep(paste0("Reader", x$reference.Reader), x$n.reader-1), " vs ", rownames(temp_CLR_spe))
    }
